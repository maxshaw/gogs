...
func runHookPreReceive(c *cli.Context) error {
	...
	isWiki := strings.Contains(os.Getenv(http.RouteEnvRepoCustomHooksPath), ".wiki.git/")
	...
	for scanner.Scan() {
		...
		// Branch protection
		repoID := com.StrTo(os.Getenv(http.RouteEnvRepoId)).MustInt64()
		...
		userID := com.StrTo(os.Getenv(http.RouteEnvAuthUserId)).MustInt64()
		...
		output, err := git.NewCommand("rev-list", "--max-count=1", oldCommitID, "^"+newCommitID).
			RunInDir(models.RepoPath(os.Getenv(http.RouteEnvRepoOwnerName), os.Getenv(http.RouteEnvRepoName)))
		if err != nil {
	...
	customHooksPath := filepath.Join(os.Getenv(http.RouteEnvRepoCustomHooksPath), "pre-receive")
	...
	hookCmd.Dir = models.RepoPath(os.Getenv(http.RouteEnvRepoOwnerName), os.Getenv(http.RouteEnvRepoName))
...
func runHookUpdate(c *cli.Context) error {
	...
	customHooksPath := filepath.Join(os.Getenv(http.RouteEnvRepoCustomHooksPath), "update")
	if !com.IsFile(customHooksPath) {
		return nil
	}
...
func runHookPostReceive(c *cli.Context) error {
	...
	for scanner.Scan() {
		...
		options := models.PushUpdateOptions{
			...
			PusherID:     com.StrTo(os.Getenv(http.RouteEnvAuthUserId)).MustInt64(),
			PusherName:   os.Getenv(http.RouteEnvAuthUserName),
			RepoUserName: os.Getenv(http.RouteEnvRepoOwnerName),
			RepoName:     os.Getenv(http.RouteEnvRepoName),
		}
		...
		reqURL := setting.LocalURL + options.RepoUserName + "/" + options.RepoName + "/tasks/trigger?branch=" +
			...
			"&secret=" + os.Getenv(http.RouteEnvRepoOwnerSaltMd5) +
			"&pusher=" + os.Getenv(http.RouteEnvAuthUserId)
	...
	customHooksPath := filepath.Join(os.Getenv(http.RouteEnvRepoCustomHooksPath), "post-receive")
	if !com.IsFile(customHooksPath) {
		return nil
	}
